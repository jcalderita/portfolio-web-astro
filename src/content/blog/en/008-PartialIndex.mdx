...
A **partial index** is an index created only on a subset of rows in a table, defined by a specific <span class="high">WHERE</span> condition. Instead of indexing all rows, the index only includes those that meet certain criteria, which can improve performance and reduce space usage.
...
It's also possible to create partial indexes involving multiple columns:
...
By default, Vapor doesn't offer direct support for creating partial indexes, since standard index generation doesn't include the ability to add a <span class="high">WHERE</span> clause in the index definition.

The following code snippet creates a normal index, either on one or multiple fields, but doesn't allow specifying a condition for a partial index:
...
While effective, this approach loses the advantage of the abstraction and safety that Vapor offers when building migrations and database schemas through Swift code.

---

## Solution

Analyzing Vapor's API, I realized that the <span class="high">SQLCreateIndexBuilder</span> builder accepts an optional <span class="high">predicate</span>. If this is not <span class="high">nil</span>, it adds a <span class="high">WHERE</span> clause to the index.
...
This code allows calling the <span class="high">where</span> method for one or multiple fields, specifying whether you want a partial index for <span class="high">NULL</span> or <span class="high">NOT NULL</span> values. If no value is provided for <span class="high">partialIndex</span>, the index is returned without a predicate, behaving like a normal index.

The logic consists of creating a list of <span class="high">SQLBinaryExpression</span> binary expressions for each column, combining them with the <span class="high">AND</span> logical operator and assigning the result as the index predicate.

---

## Result

With this extension, I can now easily create partial indexes in Vapor, adding only one line to the original code:
...
Where <span class="high">$0.partial</span> is an optional value indicating the desired partial index type <span class="high">null</span> or <span class="high">not null</span>.

---

## Final Notes

> This approach is specifically designed for partial indexes based on the presence or absence of <span class="high">NULL</span> values in columns. However, the implementation can be adapted to support other conditions and more complex use cases by simply modifying the predicate construction.
